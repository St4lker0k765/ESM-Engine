...
//	desc.name			= xr_strlwr(xr_strdup(name));
    ...
    desc.modif			= modif & (~static_cast<u32>(0x3));
...
//	if file already exist - update info
	...
	while (temp[0]) 
	{
		...
		if (!exist(path))	
		{
            ...
            desc.modif			= static_cast<u32>(-1);
            std::pair<files_it,bool> IFiles = files.insert(desc); 

            R_ASSERT(IFiles.second);
		}
...
void CLocatorAPI::ProcessArchive(LPCSTR _path, LPCSTR base_path)
{
	...
	while (!hdr->eof())
	{
		...
		Register		(full,static_cast<u32>(vfs),crc,ptr,size_real,size_compr,0);
	}
	hdr->close			();
...
{
	_finddata_t& F	= *static_cast<_finddata64i32_t*>(_F);
	...
	if (F.attrib&_A_SUBDIR) {
		...
		if (0==xr_strcmp(F.name,"..")) return;
		strcat		(N,"\\");
		Register	(N,0xffffffff,0,0,F.size,F.size,static_cast<u32>(F.time_write));
		Recurse		(N);
	} else {
		if (strext(N) && 0==strncmp(strext(N),".db",3))		ProcessArchive	(N);
		else												Register		(N,0xffffffff,0,0,F.size,F.size,static_cast<u32>(F.time_write));
	}
}
...
bool CLocatorAPI::Recurse		(const char* path)
{
	...
	_finddata_t		*buffer = static_cast<_finddata64i32_t*>(_alloca(count * sizeof(_finddata_t)));
...
void CLocatorAPI::_initialize	(u32 flags, LPCSTR target_folder, LPCSTR fs_name)
{
	...
	}else
	{
		...
		while(!pFSltx->eof())
		{
			...
			auto* P			= xr_new<FS_Path>((p_it!=pathes.end())?p_it->second->m_Path:root,lp_add,lp_def,lp_capt,fl);
...
void CLocatorAPI::_destroy		()
{
	...
	{
		char* str	= const_cast<LPSTR>(I->name);
		xr_free		(str);
	}
	files.clear		();
	for				(auto p_it=pathes.begin(); p_it!=pathes.end(); p_it++)
    {
		char* str	= const_cast<LPSTR>(p_it->first);
		xr_free		(str);
...
{
	auto*r = xr_new<CFileStreamReader>();
...
void CLocatorAPI::file_from_archive	(IReader *&R, LPCSTR fname, const file &desc)
{
	...
	u32 sz						= (end-start);
	u8* ptr						= static_cast<u8*>(MapViewOfFile(A.hSrcMap, FILE_MAP_READ, 0, start, sz)); VERIFY3(ptr,"cannot create file mapping on file",fname);
...
IWriter* CLocatorAPI::w_open	(LPCSTR path, LPCSTR _fname)
{
	...
	auto* W 	= xr_new<CFileWriter>(fname,false); 
	return W;
}
...
IWriter* CLocatorAPI::w_open_ex	(LPCSTR path, LPCSTR _fname)
{
	...
	xr_strlwr(fname);
	if (path&&path[0]) update_path(fname,path,fname);
	auto* W 	= xr_new<CFileWriter>(fname,true); 
	return W;
}
...
void	CLocatorAPI::w_close(IWriter* &S)
{
	if (S){
		...
		if(bReg)
		{
			struct _stat st;
			_stat		(fname,&st);
			Register	(fname,0xffffffff,0,0,st.st_size,st.st_size,static_cast<u32>(st.st_mtime));
		}
    }
}
...
void CLocatorAPI::file_delete(LPCSTR path, LPCSTR nm)
{
    ...
    if (I!=files.end()){
	    // remove file
    	unlink			(I->name);
		auto* str		= const_cast<LPSTR>(I->name);
		xr_free			(str);
	    files.erase		(I);
    }
}
...
FS_Path* CLocatorAPI::append_path(LPCSTR path_alias, LPCSTR root, LPCSTR add, BOOL recursive)
{
	...
	auto* P		= xr_new<FS_Path>(root,add,static_cast<LPCSTR>(nullptr),static_cast<LPCSTR>(nullptr),0);
...
u32 CLocatorAPI::get_file_age(LPCSTR nm)
{
    ...
    return (I!=files.end())?I->modif:static_cast<u32>(-1);
}
