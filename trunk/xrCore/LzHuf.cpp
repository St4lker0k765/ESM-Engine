...
{
    int  i, c, lenEncode, r, s, last_match_length;
    ...
    for (i = s; i < r; i++)
        text_buf[i] = 0x20;
    for (lenEncode = 0; lenEncode < F && (c = fs._getb()) != EOF; lenEncode++)
        text_buf[r + lenEncode] = (unsigned char)c;
    textsize = lenEncode;
    for (i = 1; i <= F; i++)
        InsertNode(r - i);
    InsertNode(r);
    do {
        if (match_length > lenEncode)
            match_length = lenEncode;
