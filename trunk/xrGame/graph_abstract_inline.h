...
{
	stream.open_chunk			(0);
	stream.w_u32				((u32)CAbstractGraph::vertices().size());
	stream.close_chunk			();
	...
	stream.open_chunk			(2);
	{
		auto	i_const_vect = vertices().cbegin();
		auto	e_const_vect = vertices().cend();
		for ( ; i_const_vect != e_const_vect; ++i_const_vect) {
			if ((*i_const_vect).second->edges().empty())
				continue;
